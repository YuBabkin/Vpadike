...
# Путь к файлу лога ошибок: лежит рядом с базой данных (instance/error.log),
# чтобы его можно было прочитать через FTP с сервера.
ERROR_LOG_PATH = os.path.join(BASE_DIR, 'instance', 'error.log')
...
def log_unhandled_exception(error):
    ...
    мы записываем полный трейсбек в файл instance/error.log.
